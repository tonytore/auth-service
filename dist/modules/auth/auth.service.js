...
exports.authService = {
    register: async ({ payload }) => {
        const existingUser = await auth_repository_1.authRepository.findByEmail(payload.email);
        if (existingUser) {
            throw new custom_error_handler_1.BadRequestError("User already exists", "AuthService");
        }
    ...
    login: async ({ email, password }) => {
        ...
        if (!user) {
            throw new custom_error_handler_1.UnauthenticatedError("Invalid credentials", "AuthService");
        }
        const isValid = await bcrypt_1.default.compare(password, user.password);
        if (!isValid) {
            throw new custom_error_handler_1.UnauthenticatedError("Invalid credentials", "AuthService");
        }
        const accessToken = jsonwebtoken_1.default.sign({ userId: user.id, role: user.role }, app_config_1.default.ACCESS_TOKEN_SECRET, { expiresIn: app_config_1.default.ACCESS_TOKEN_EXPIRY });
